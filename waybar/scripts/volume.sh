...
type="/home/arch12/.local/share/rofi/themes/"
style='applets.rasi'
...
if [[ "$?" == 0 ]]; then
	active="-a 1"
	stext='Mute'      # <-- if it's on, the action is to mute
	sicon=''
else
	urgent="-u 1"
	stext='Unmute'    # <-- if it's off, the action is to unmute
	sicon=''
fi
